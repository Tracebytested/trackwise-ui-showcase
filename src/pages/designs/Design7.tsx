...
import { 
  ...
  ArrowLeft, RefreshCw, UserPlus, Mail, BarChart3, Cog, Zap
} from 'lucide-react';

// Rose Glow - Top nav with pink/rose gradient (inspired by Design4 layout)
const Design7 = () => {
  const devices = [
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-rose-950 to-slate-900 text-white">
      {/* Top Navigation */}
      <header className="bg-slate-900/80 backdrop-blur-lg border-b border-rose-500/20 sticky top-0 z-50">
        <div className="flex items-center justify-between px-6 h-16">
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="text-rose-400 text-xs ml-2">Installer</span>
              </div>
            </div>
              ...
              {[
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm transition-all ${
                    item.active ? 'bg-rose-500/20 text-rose-400' : 'text-gray-400 hover:text-white hover:bg-white/5'
                  }`}
                >
                  <item.icon className="w-4 h-4" />
                  <span>{item.label}</span>
                  {item.badge && <span className="bg-pink-500 text-xs px-1.5 py-0.5 rounded-full">{item.badge}</span>}
                </button>
              ))}
            </nav>
          </div>
          ...
          <div className="flex items-center gap-4">
            <Link to="/" className="text-gray-400 hover:text-rose-400 text-sm flex items-center gap-1">
              <ArrowLeft className="w-4 h-4" /> Back
            </Link>
            <button className="px-4 py-2 bg-gradient-to-r from-rose-500 to-pink-600 rounded-lg text-sm font-medium flex items-center gap-2">
      ...
      <main className="p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Search devices..." className="pl-10 pr-4 py-2.5 bg-slate-800/50 border border-rose-500/20 rounded-xl text-sm w-64 focus:outline-none focus:border-rose-500" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          {[
            ...
            <div key={stat.label} className="p-4 rounded-xl bg-slate-800/50 border border-rose-500/10">
        ...
        {/* Table */}
        <div className="rounded-2xl bg-slate-800/50 border border-rose-500/10 overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {devices.map((device) => (
                <tr key={device.id} className="border-t border-slate-700/30 hover:bg-rose-500/5 transition-colors">
                  ...
                  <td className="px-4 py-4">
                    {device.assigned ? <span className="text-rose-400">{device.assigned}</span> : <span className="text-gray-500">Unassigned</span>}
                  </td>
                  <td className="px-4 py-4 text-gray-400 text-xs">{device.lastSeen}</td>
                  <td className="px-4 py-4 text-rose-400">{device.satFix}</td>
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-1">
                      <button className="p-1.5 hover:bg-rose-500/20 rounded"><BarChart3 className="w-4 h-4 text-gray-400" /></button>
                      <button className="p-1.5 hover:bg-rose-500/20 rounded"><Cog className="w-4 h-4 text-gray-400" /></button>
                      <button className="p-1.5 hover:bg-rose-500/20 rounded"><Mail className="w-4 h-4 text-gray-400" /></button>
                      {device.assigned ? (
                        <button className="p-1.5 hover:bg-rose-500/20 rounded"><RefreshCw className="w-4 h-4 text-gray-400" /></button>
                      ) : (
