...
import { 
  ...
  Mail, BarChart3, Cog, Server, Activity
} from 'lucide-react';

// Emerald Dashboard - Left sidebar with green accent (inspired by Design5/Design13)
const Design2 = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-50">
        <div className="flex items-center justify-between px-6 h-14">
          <div className="flex items-center gap-6">
            ...
            <Link to="/" className="text-slate-400 hover:text-slate-600 text-sm">← Back to Designs</Link>
          </div>
          <div className="flex items-center gap-3">
            <button className="p-2 hover:bg-slate-100 rounded-lg"><Bell className="w-5 h-5 text-slate-500" /></button>
            <button className="p-2 hover:bg-slate-100 rounded-lg"><Settings className="w-5 h-5 text-slate-500" /></button>
            <button className="px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-medium flex items-center gap-2 hover:bg-emerald-700">
      ...
      <div className="flex">
        {/* Sidebar */}
        <aside className="w-56 min-h-[calc(100vh-56px)] bg-white border-r border-slate-200 p-4">
        ...
        <main className="flex-1 p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="relative">
              ...
              <input type="text" placeholder="Search devices..." className="pl-10 pr-4 py-2 border border-slate-300 rounded-lg text-sm w-64 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 gap-4 mb-6">
            {[
              { label: 'Total', value: '24', bg: 'bg-emerald-50', text: 'text-emerald-700', icon: Cpu },
              { label: 'Online', value: '18', bg: 'bg-green-50', text: 'text-green-700', icon: Wifi },
              ...
              <tbody>
                {devices.map((device) => (
                  <tr key={device.id} className="border-t border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 text-emerald-600">{device.assigned || <span className="text-slate-400">—</span>}</td>
                    <td className="px-6 py-4 text-slate-500">{device.lastSeen}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1">
                        <button className="p-1.5 hover:bg-slate-100 rounded"><BarChart3 className="w-4 h-4 text-slate-400" /></button>
                        <button className="p-1.5 hover:bg-slate-100 rounded"><Cog className="w-4 h-4 text-slate-400" /></button>
