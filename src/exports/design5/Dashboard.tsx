...
import { 
  ArrowLeft, MapPin, Gauge, Satellite, Signal, Battery, Car, Ship, Caravan, 
  Navigation, Bell, Settings, LogOut, Search, ChevronRight 
} from 'lucide-react';
...
// Import your map image - update path as needed
...
const Dashboard = ({ onBack, onSignOut, mapImage }: DashboardProps) => {
  const { devices, loading, totalCount, activeCount, movingCount, addDevice } = useDevices();
      ...
      <header className="bg-white border-b border-slate-200 shadow-sm">
        <div className="px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {onSignOut && (
              <button onClick={onSignOut} className="p-2 hover:bg-slate-100 rounded-lg">
        ...
        <main className="flex-1 p-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-6">
            ...
            <div className="h-96 relative bg-slate-200">
              {mapImage ? (
                <img src={mapImage} alt="Fleet map" className="w-full h-full object-cover" />
              ) : (
                <div className="w-full h-full flex items-center justify-center text-slate-400">
                  <div className="text-center">
                    <MapPin className="w-12 h-12 mx-auto mb-2" />
                    <p>Map placeholder - pass mapImage prop</p>
                  </div>
                </div>
              )}
              {selectedDevice && (
